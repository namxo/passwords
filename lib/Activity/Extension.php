...
namespace OCA\Passwords\Activity;

use OCA\Passwords\Manager;
...
class Extension implements IExtension {
	...
	public function getTypeIcon($type) {
		return $type === 'passwords' ? 'icon-info' : false;
	}
	...
	public function translate($app, $text, $params, $stripPath, $highlightParams, $languageCode) {
		if ($app === 'passwords') {
			$l = $this->languageFactory->get('passwords', $languageCode);
			...
			$params[] = '<parameter>' . $password['subject'] . '</parameter>';

			try {
				if ($password['author'] === $this->activityManager->getCurrentUserId()) {
					array_shift($params);
	...
	public function getSpecialParameterList($app, $text) {
		if ($app === 'passwords'&& strpos($text, 'passwordsubject#') === 0) {
			return [
				0 => 'username',
			];
		}
